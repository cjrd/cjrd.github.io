...
(function(d3, topojson){
  ...
  /* inset initializaiton */
  var svgInset = d3.select("body").append("svg")
        .attr("id", "inset")
  ...
  /*
   * does the query text (qText) fuzzily match the baseText?
   */
  ...
  function textKeyUp(){
    ...
    if ( arrowUp || arrowDown){
      ...
      var lis = d3Ul.selectAll("li"),
          numLi = lis[0].length;
      if ( numLi > 1){
        d3Ul.select(".selected").classed("selected", false);
        if (arrowUp){
          selNum = --selNum < 0 ? numLi-1 : selNum;
        } else if (arrowDown){
          selNum = ++selNum >= numLi ? 0 : selNum;
        }
        d3.select(lis[0][selNum]).classed("selected", true);
        d3.event.preventDefault();
        return;
      }
    } else if (enterPressed){
      inText = d3Ul.select(".selected").text();
    ...
    // clear list
    d3Ul.html("");
    // fill list with top 3 fuzzy matches
    if (inTextLen > 0){
      circles.each(function(d){
        d3this = d3.select(this);
        fmatch = fuzzyMatch(inText, d.name);
        if (fmatch){
